Shader "Langvv/Hypnotic_Scroll_X_Grayscale"
{
	Properties 
	{
		...
		_HipSpeed("Hipnotic Speed", Float) = 1
	...
	SubShader 
	{
		...
		#pragma target 2.0
			...
			void surf (Input IN, inout EditorSurfaceOutput o) {
				...
				float4 Multiply5=_Time * _MainSpeed.xxxx;
				float4 UV_Pan0=float4((IN.uv_MainTexture.xyxy).x + Multiply5.x,(IN.uv_MainTexture.xyxy).y,(IN.uv_MainTexture.xyxy).z,(IN.uv_MainTexture.xyxy).w);
				float4 Tex2D1=tex2D(_MainTexture,UV_Pan0.xy);
				float4 Multiply6=Tex2D1 * _MainColor;
